...
function EditItem() {
    ...
    useEffect(() => {
        axios.get('http://localhost:5050/api/warehouses')
        .then(response => {
    ...
    const handleSubmit = (e) => {
        ...
        if (isFormValid()) {
            ...
            axios
            .put(
                `http://localhost:5050/api/inventories/` + id,
                itemData
            )
            .then((response) => {
                ...
                >
                    ...
                    <div className="edit-item__details edit-item__details--border ">
                        ...
                        <select
                            ...
                            className={`edit-item__input ${hasError("store") ? "edit-item__input--error" : ""
                                }`}
                        >
                            <option value="">Please select</option>
                            {storeArray.map((store) => {
                                return (
                                    <option key={store.id} value={store.id} select='true'>{store.warehouse_name}</option>)
                            })}
                        </select>
                        {hasError("store") && (
                            <span className="edit-item__error-message">
